...
public class Method_15 extends MethodController {
	...
	private void stoneIt(String inputFileName, String outputFileName) {
		...
		try {
			...
			int flag = flags_byte[0] + 48, flag_1 = flags_byte[1] + 171, flag_2 = flags_byte[2] - 100,
					flag_3 = flags_byte[3] - 42, flag_control = (Math.abs(flags_byte[4] % 8) + 1);
	...
	void setPassword(String passWord) {
		...
		for (int i = 0; i < flags.length; i++) {
			...
			if (counter == 1) {
				flags_byte[0] = (byte) (flags_byte[0] + flags[i] + 1);
				counter = 2;
			} else if (counter == 2) {
				flags_byte[1] = (byte) (flags_byte[1] + flags[i] + 3);
				counter = 3;
			} else if (counter == 3) {
				flags_byte[2] = (byte) (flags_byte[2] + flags[i] + 4);
				counter = 4;
			} else if (counter == 4) {
				flags_byte[3] = (byte) (flags_byte[3] + flags[i] + 2);
				counter = 1;
			}

		}
		flags_byte[4] = (byte) ((flags_byte[1] + flags_byte[2] + flags_byte[3]) / 4);
